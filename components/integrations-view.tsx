    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          <h2 className="text-3xl font-bold text-slate-900 tracking-tight">System Integrations</h2>
          <p className="text-slate-500">Connect your stack to the PMO-Ops command engine.</p>
        </div>
        <button className="flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-xl font-bold uppercase tracking-widest text-xs hover:bg-slate-800 transition-all shadow-lg shadow-slate-200">
          <Plus size={16} />
          Request Integration
        </button>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input 
            ...
            className="w-full pl-12 pr-4 py-4 rounded-xl bg-white border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-sm font-medium"
          />
        </div>
        <div className="flex gap-2 overflow-x-auto pb-2 sm:pb-0 no-scrollbar">
          {['All', ...Array.from(new Set(integrations.map(i => i.category)))].map((filter) => (
            <button 
              ...
              className={`px-6 py-4 border rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all whitespace-nowrap ${
                activeFilter === filter ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'
              }`}
            >
              {filter}
            </button>
        ...
        {filteredIntegrations.map((app) => (
          ...
          >
            <div className="flex items-start justify-between mb-6">
              <div className={`w-12 h-12 rounded-xl ${app.color} flex items-center justify-center shadow-sm`}>
                <app.icon size={24} />
              </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{app.category}</p>
              </div>
            ...
            <div className="mt-6 pt-5 border-t border-slate-50 flex items-center justify-between">
              <button className="text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-slate-900 transition-colors flex items-center gap-2">
                <Settings2 size={14} />
                Configure
              </button>
      ...
      {/* Security Banner */}
      <div className="bg-slate-900 rounded-2xl p-8 text-white flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden">
        ...
        <div className="flex items-center gap-6 relative z-10">
          ...
          <div>
            ...
            <p className="text-slate-400 max-w-md text-sm mt-1">All integrations use OAuth 2.0 and AES-256 encryption. Your data remains private and secure.</p>
          </div>
        </div>
        <button className="px-8 py-4 bg-white text-slate-900 rounded-xl font-bold uppercase tracking-widest text-xs hover:bg-slate-100 transition-colors relative z-10 shadow-xl">
          Security Audit Log
        </button>
      </div>
    </div>
